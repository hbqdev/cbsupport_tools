...
DIR_TICKETS="${DIR_TICKETS:-$HOME/tickets}"
...
preflight() {
    ...
    _check_dep s3dl  "pip install git+https://github.com/couchbaselabs/s3dl.git" || fail=1
    if [ "$fail" -ne 0 ]; then
...
cbd() {
    ...
    wait "$pid" || { echo "s3dl download failed" >&2; return 1; }
